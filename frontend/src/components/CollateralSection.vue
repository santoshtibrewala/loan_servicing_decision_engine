...
<template>
  ...
  >
    <div class="mb-[14px] flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
      <span class="text-[0.92rem] text-[#546a80]">{{ collateral.length }} collateral records</span>
      <button class="rounded-[12px] border border-[#adc1d4] bg-white px-[14px] py-[9px] font-semibold text-[#0d3e68]" @click="emit('add-collateral')">
        Add collateral
      </button>
    </div>
    <div class="grid gap-[14px]">
      ...
      >
        <div class="mb-[14px] flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
          <div>
            <h3 class="m-0 text-[1.2rem] font-bold text-[#0f1c2e]">{{ item.propertyId || `Collateral ${index + 1}` }}</h3>
            <span class="text-[0.86rem] font-[550] text-[#293c4f]">{{ lookupLabel(collateralTypeOptions, item.propertyType) }}</span>
          </div>
          <button class="rounded-[12px] border border-[#adc1d4] bg-white px-[14px] py-[9px] font-semibold text-[#b53d2f]" @click="emit('remove-collateral', index)">
            Remove
          </button>
        </div>
        <div class="mb-[14px] grid gap-[10px] md:grid-cols-2">
          <div class="rounded-[12px] border border-[#d4dee7] bg-[#eef4f9] p-3">
            <span class="mb-1 block text-[0.8rem] text-[#5f7387]">Market value</span>
            <strong class="text-[#143b5f]">{{ currencyCompact(item.marketValue) }}</strong>
          </div>
          <div class="rounded-[12px] border border-[#d4dee7] bg-[#eef4f9] p-3">
            <span class="mb-1 block text-[0.8rem] text-[#5f7387]">Recoverable value</span>
            <strong class="text-[#143b5f]">{{ currencyCompact(item.recoverableValue) }}</strong>
          </div>
        </div>
