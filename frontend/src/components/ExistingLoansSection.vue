      ...
      >
        ...
        <div class="mb-[14px] grid gap-[10px] md:grid-cols-2 xl:grid-cols-4">
          ...
          <div class="rounded-[12px] border border-[#d4dee7] bg-[#eef4f9] p-3">
            ...
            <strong class="text-[#143b5f]">{{
              currencyCompact(
                Number(loan.principal ?? 0) + Number(loan.accruedInterest ?? 0),
              )
            }}</strong>
          </div>
        </div>
        <div class="grid gap-[14px] md:grid-cols-2 xl:grid-cols-4">
          ...
          >
          <label class="grid gap-1.5 font-semibold text-[#293c4f]"
            >Principal<FormattedNumberInput
          ...
          /></label>
          <label class="grid gap-1.5 font-semibold text-[#293c4f]"
            >Accrued interest<FormattedNumberInput
              v-model="loan.accruedInterest"
              kind="amount"
              :min="0"
              class="w-full rounded-[10px] border-2 border-[#0e416a] bg-white px-[14px] py-3 text-[#17334d]"
          /></label>
          <label class="grid gap-1.5 font-semibold text-[#293c4f]"
            >Existing rate<FormattedNumberInput
              v-model="loan.existingRate"
              kind="percent"
              :scale="100"
              :decimals="2"
              :min="0"
          ...
          /></label>
          <label class="grid gap-1.5 font-semibold text-[#293c4f]"
            >First-year payment<FormattedNumberInput
              v-model="loan.firstYearPayment"
              kind="amount"
              :min="0"
              class="w-full rounded-[10px] border-2 border-[#0e416a] bg-white px-[14px] py-3 text-[#17334d]"
          /></label>
